...
try:
    ...
    raise SystemExit(
        "Missing dependency 'diffusers' in the active environment.\n"
        "Install with: python -m pip install -U diffusers transformers accelerate safetensors"
    ) from exc
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Smoke-test for the LTX-Video adapter."
    )
    parser.add_argument(
        "--variant",
        type=str,
...
def main() -> int:
    ...
    try:
        adapter = create_adapter("ltx_video", **adapter_kwargs)
