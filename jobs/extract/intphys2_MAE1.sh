...
#SBATCH --gpus=1
#SBATCH --job-name=IntPhys2MAEExtract
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=02:00:00
#SBATCH --output=jobs/out/intphys2_MAE1_extract_%j.out
#SBATCH --error=jobs/out/intphys2_MAE1_extract_%j.err
...
AUTO_DOWNLOAD_IF_MISSING="${AUTO_DOWNLOAD_IF_MISSING:-false}"
...
echo "Step: extract.intphys2 (${BACKBONE_NAME} activations)"
cmd=(
  ...
  feature_cache.split_names="${EXTRACT_SPLITS}"
...
if [[ "${BACKBONE_NAME}" == jepa_* ]]; then
  cmd+=("+backbone.kwargs.checkpoint_path=${CKPT}")
fi
...
if [[ -n "${BACKBONE_EXTRA_OVERRIDES}" ]]; then
  # shellcheck disable=SC2206
  extra_overrides=( ${BACKBONE_EXTRA_OVERRIDES} )
  cmd+=("${extra_overrides[@]}")
fi
