...
#
# Layout:
#   artifacts/probes/${GROUP_SUBDIR}/layer_<layer>/lr_<tag>/
#
# The script also incrementally rebuilds:
#   artifacts/probes/${GROUP_SUBDIR}/train_eval_summary.json
#   artifacts/probes/${GROUP_SUBDIR}/train_eval_summary.csv
# keeping, for each layer, the best completed LR run selected on validation.
#
# Quick check after completion:
#   column -s, -t < artifacts/probes/mvp_probe_temporal_attn_jepa_v2_1_vitG_384_lr_matrix/train_eval_summary.csv
...
RUN_ROOT="${REPO_ROOT}/artifacts/probes/${RUN_SUBDIR}"
GROUP_ROOT="${REPO_ROOT}/artifacts/probes/${GROUP_SUBDIR}"
TRAIN_ROOT="${REPO_ROOT}/artifacts/probes/${TRAIN_SUBDIR}"
EVAL_ROOT="${REPO_ROOT}/artifacts/results/${EVAL_SUBDIR}"
