#!/bin/bash
# Recover the original seed-42 MVP LTX-13B MLP Optuna search.
#
# One array task = one LTX noise x block slot. Submit with:
#   sbatch --array=0-39 jobs/train/mvp/recovery/ltx13b_mlp_seed42_array.sh

#SBATCH --partition=rome
#SBATCH --job-name=recover_mvp_ltx13b_mlp
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --time=48:00:00
#SBATCH --output=output/training/mvp/recovery_ltx13b_mlp/recover_mvp_ltx13b_mlp_%A_%a.out
#SBATCH --error=output/training/mvp/recovery_ltx13b_mlp/recover_mvp_ltx13b_mlp_%A_%a.err
...
OPTUNA_STUDY_NAME="recovery_seed42_mvp_ltx13b_mlp_slot${SLOT_PADDED}"
...
RUNNER="${REPO_ROOT}/jobs/train/mvp/mlp/run_train.sh"
OUTPUT_SUBDIR="recovery_seed42_mvp_ltx13b_mlp_slot${SLOT_PADDED}_${SLOT_LABEL}"
