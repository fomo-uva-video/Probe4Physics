#!/bin/bash
# Train IntPhys2 temporal_attn probes with ltx_video features over a fixed
# (slot, learning-rate) matrix.
#
# Usage:
#   sbatch jobs/train/intphys2/temporal_attn/ltx_video_lr_matrix.sh
#
# Default matrix:
#   slots = 34, 36, 38, 39
#   lrs   = 5e-4, 1e-4, 5e-5, 1e-5
#
# LTX slot ids follow docs/ltx_video_slot_map.md (noise-major, depth-minor):
#   34 -> noise_0.2_block_24
#   36 -> noise_0.2_block_48
#   38 -> noise_0.1_block_24
#   39 -> noise_0.1_block_36
#
# Layout:
...
#   column -s, -t < artifacts/probes/intphys2/intphys2_probe_temporal_attn_ltx_video_ltxv_13b_0_9_8_distilled_lr_matrix/train_eval_summary.csv

#SBATCH --partition=gpu_h100
#SBATCH --job-name=intphys2_ltx_video_attn_lr_matrix
#SBATCH --array=0-15
#SBATCH --ntasks=1
#SBATCH --gpus=1
...
#SBATCH --output=output/training/intphys2/attention/intphys2_ltx_video_attn_lr_matrix_%A_%a.out
#SBATCH --error=output/training/intphys2/attention/intphys2_ltx_video_attn_lr_matrix_%A_%a.err

set -euo pipefail

DATASET_NAME="${DATASET_NAME:-intphys2}"
PROBE_NAME="${PROBE_NAME:-temporal_attn}"
BACKBONE_NAME="${BACKBONE_NAME:-ltx_video}"
BACKBONE_VARIANT="${BACKBONE_VARIANT:-ltxv_13b_0_9_8_distilled}"
MATRIX_LAYERS="${MATRIX_LAYERS:-34,36,38,39}"
...
GROUP_SUBDIR="${GROUP_SUBDIR:-intphys2_probe_temporal_attn_ltx_video_ltxv_13b_0_9_8_distilled_lr_matrix}"
WANDB_GROUP="${WANDB_GROUP:-intphys2_temporal_attn_ltx_video_lr_matrix}"
