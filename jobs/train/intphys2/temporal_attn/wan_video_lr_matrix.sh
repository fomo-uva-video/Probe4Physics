#!/bin/bash
# Train IntPhys2 temporal_attn probes with wan_video features over a fixed
# (slot, learning-rate) matrix.
#
# Usage:
#   sbatch jobs/train/intphys2/temporal_attn/wan_video_lr_matrix.sh
#
# Default matrix:
#   slots = 21, 36, 38, 39
#   lrs   = 5e-4, 1e-4, 5e-5, 1e-5
#
# Wan slot ids use noise-major, depth-minor ordering documented in
# docs/wan_video_probe_slots.md. The examples below correspond to the current
# default WAN variant (14B):
#   21 -> noise_0.5_block_10
#   36 -> noise_0.2_block_40
#   38 -> noise_0.1_block_20
#   39 -> noise_0.1_block_30
#
# Layout:
#   artifacts/probes/intphys2/${GROUP_SUBDIR}/layer_<slot>/lr_<tag>/
#
...
# keeping, for each slot, the best completed LR run selected on val VOE.
#
# Quick check after completion:
#   column -s, -t < artifacts/probes/intphys2/intphys2_probe_temporal_attn_wan_video_lr_matrix/train_eval_summary.csv

#SBATCH --partition=gpu_h100
#SBATCH --job-name=intphys2_wan_video_attn_lr_matrix
#SBATCH --array=0-15
#SBATCH --ntasks=1
#SBATCH --gpus=1
...
#SBATCH --output=output/training/intphys2/attention/intphys2_wan_video_attn_lr_matrix_%A_%a.out
#SBATCH --error=output/training/intphys2/attention/intphys2_wan_video_attn_lr_matrix_%A_%a.err

set -euo pipefail

DATASET_NAME="${DATASET_NAME:-intphys2}"
PROBE_NAME="${PROBE_NAME:-temporal_attn}"
BACKBONE_NAME="${BACKBONE_NAME:-wan_video}"
BACKBONE_VARIANT="${BACKBONE_VARIANT:-}"
MATRIX_LAYERS="${MATRIX_LAYERS:-21,36,38,39}"
...
GROUP_SUBDIR="${GROUP_SUBDIR:-intphys2_probe_temporal_attn_wan_video_lr_matrix}"
WANDB_GROUP="${WANDB_GROUP:-intphys2_temporal_attn_wan_video_lr_matrix}"
