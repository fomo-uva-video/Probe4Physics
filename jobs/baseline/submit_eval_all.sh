#!/bin/bash
# Submit baseline evaluation jobs for MVP and IntPhys2 across all configured backbones.
#
# Optional filters:
...
for dataset in "${DATASETS[@]}"; do
  ...
  for baseline in "${BASELINES[@]}"; do
    ...
    for backbone in "${BACKBONES[@]}"; do
      ...
      job_name="${dataset}_${baseline}_${backbone}_baseline_eval"
      cmd=(
        sbatch
        "--job-name=${job_name}"
