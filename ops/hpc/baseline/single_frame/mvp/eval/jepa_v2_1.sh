#!/bin/bash
# Evaluate the MVP single-frame baseline with V-JEPA v2.1 features.
#
...
#   extract_jid=$(sbatch --parsable ops/hpc/baseline/single_frame/mvp/extract/jepa_v2_1.sh)
#   sbatch --dependency=afterok:${extract_jid} --export=ALL,PROBE_OUTPUT_DIR=/scratch-shared/${USER}/probe4physics/artifacts/probes/mvp ops/hpc/baseline/single_frame/mvp/eval/jepa_v2_1.sh
# The mkdir is required because Slurm opens stdout/stderr before the script runs.
#
#SBATCH --partition=rome
#SBATCH --job-name=mvp_sf_eval_jepa_v2_1
#SBATCH --ntasks=1
