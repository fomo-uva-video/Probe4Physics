#!/bin/bash
# Extract the MVP single-frame baseline with VideoMAE v2 features.
#
...
#   extract_jid=$(sbatch --parsable ops/hpc/baseline/single_frame/mvp/extract/videomae_v2.sh)
#   sbatch --dependency=afterok:${extract_jid} --export=ALL,PROBE_OUTPUT_DIR=/scratch-shared/${USER}/probe4physics/artifacts/probes/mvp ops/hpc/baseline/single_frame/mvp/eval/videomae_v2.sh
# The mkdir is required because Slurm opens stdout/stderr before the script runs.
#
#SBATCH --partition=gpu_a100
#SBATCH --gpus=1
#SBATCH --job-name=mvp_sf_extract_videomae_v2
#SBATCH --ntasks=1
