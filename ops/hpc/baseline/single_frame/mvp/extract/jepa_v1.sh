#!/bin/bash
# Extract the MVP single-frame baseline with V-JEPA v1 features.
#
...
#   extract_jid=$(sbatch --parsable ops/hpc/baseline/single_frame/mvp/extract/jepa_v1.sh)
#   sbatch --dependency=afterok:${extract_jid} --export=ALL,PROBE_OUTPUT_DIR=/scratch-shared/${USER}/probe4physics/artifacts/probes/mvp ops/hpc/baseline/single_frame/mvp/eval/jepa_v1.sh
# The mkdir is required because Slurm opens stdout/stderr before the script runs.
#
#SBATCH --partition=gpu_a100
#SBATCH --gpus=1
#SBATCH --job-name=mvp_sf_extract_jepa_v1
#SBATCH --ntasks=1
