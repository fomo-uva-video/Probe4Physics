...
The layout is intentionally the same style as `ops/hpc/train`: choose the baseline,
...
```bash
sbatch ops/hpc/baseline/single_frame/mvp/extract/jepa_v1.sh
sbatch ops/hpc/baseline/single_frame/mvp/eval/jepa_v1.sh
```

For a quick extraction smoke test:

```bash
MODE=smoke sbatch ops/hpc/baseline/frame_shuffling/intphys2/extract/videomae.sh
```
...
```bash
PROBE_LAYER=32 PROBE_LAYERS=32 PROBE_CHECKPOINT_PATH=/path/to/probe_best.pt sbatch ops/hpc/baseline/single_frame/mvp/eval/jepa_v1.sh
```
