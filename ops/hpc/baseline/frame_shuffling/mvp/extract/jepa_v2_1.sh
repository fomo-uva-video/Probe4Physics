#!/bin/bash
# Extract the MVP frame-shuffling baseline with V-JEPA v2.1 features.
#
...
#   extract_jid=$(sbatch --parsable ops/hpc/baseline/frame_shuffling/mvp/extract/jepa_v2_1.sh)
#   sbatch --dependency=afterok:${extract_jid} --export=ALL,PROBE_OUTPUT_DIR=/scratch-shared/${USER}/probe4physics/artifacts/probes/mvp ops/hpc/baseline/frame_shuffling/mvp/eval/jepa_v2_1.sh
# The mkdir is required because Slurm opens stdout/stderr before the script runs.
#
#SBATCH --partition=gpu_a100
#SBATCH --gpus=1
#SBATCH --job-name=mvp_shuffle_extract_jepa_v2_1
#SBATCH --ntasks=1
