#!/bin/bash
# Extract the IntPhys2 frame-shuffling baseline with VideoMAE features.
#
...
#   extract_jid=$(sbatch --parsable ops/hpc/baseline/frame_shuffling/intphys2/extract/videomae.sh)
#   sbatch --dependency=afterok:${extract_jid} --export=ALL,PROBE_OUTPUT_DIR=/scratch-shared/${USER}/probe4physics/artifacts/probes/intphys2 ops/hpc/baseline/frame_shuffling/intphys2/eval/videomae.sh
# The mkdir is required because Slurm opens stdout/stderr before the script runs.
#
#SBATCH --partition=gpu_a100
#SBATCH --gpus=1
#SBATCH --job-name=int_shuffle_extract_videomae
#SBATCH --ntasks=1
