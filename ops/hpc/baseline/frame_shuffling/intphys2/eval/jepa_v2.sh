#!/bin/bash
# Evaluate the IntPhys2 frame-shuffling baseline with V-JEPA v2 features.
#
...
#   extract_jid=$(sbatch --parsable ops/hpc/baseline/frame_shuffling/intphys2/extract/jepa_v2.sh)
#   sbatch --dependency=afterok:${extract_jid} --export=ALL,PROBE_OUTPUT_DIR=/scratch-shared/${USER}/probe4physics/artifacts/probes/intphys2 ops/hpc/baseline/frame_shuffling/intphys2/eval/jepa_v2.sh
# The mkdir is required because Slurm opens stdout/stderr before the script runs.
#
#SBATCH --partition=rome
#SBATCH --job-name=int_shuffle_eval_jepa_v2
#SBATCH --ntasks=1
