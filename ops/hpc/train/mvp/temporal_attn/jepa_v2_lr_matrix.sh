#!/bin/bash
# Train MVP temporal_attn probes with jepa_v2 features over a fixed
# (layer, learning-rate) matrix.
#
# Usage:
#   sbatch ops/hpc/train/mvp/temporal_attn/jepa_v2_lr_matrix.sh
#
# Default matrix:
#   layers = 10, 20, 30, 40
...
#   column -s, -t < artifacts/probes/mvp/mvp_probe_temporal_attn_jepa_v2_vitg_384_lr_matrix/train_eval_summary.csv

#SBATCH --partition=gpu_a100
#SBATCH --job-name=mvp_jepa_v2_attn_lr_matrix
#SBATCH --array=0-15
#SBATCH --ntasks=1
#SBATCH --gpus=1
...
#SBATCH --output=output/training/mvp/attention/mvp_jepa_v2_attn_lr_matrix_%A_%a.out
#SBATCH --error=output/training/mvp/attention/mvp_jepa_v2_attn_lr_matrix_%A_%a.err
...
GROUP_SUBDIR="${GROUP_SUBDIR:-mvp_probe_temporal_attn_jepa_v2_vitg_384_lr_matrix}"
...
WANDB_GROUP="${WANDB_GROUP:-mvp_temporal_attn_jepa_v2_lr_matrix}"
WANDB_NAME="${WANDB_NAME:-mvp_jepa_v2_layer_${PROBE_LAYER}_lr_${LR_TAG}}"
...
train_cmd=(
  ...
  "probe.wandb.tags=[mvp,jepa_v2,temporal_attn,lr_matrix,layer_${PROBE_LAYER},lr_${LR_TAG}]"
