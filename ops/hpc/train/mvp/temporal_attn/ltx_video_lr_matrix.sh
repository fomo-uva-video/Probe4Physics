#!/bin/bash
# Train MVP temporal_attn probes with ltx_video features over a fixed
# (layer, learning-rate) matrix.
#
# Usage:
#   sbatch ops/hpc/train/mvp/temporal_attn/ltx_video_lr_matrix.sh
#
# Default matrix:
#   layers = 1, 2, ..., 40
...
#SBATCH --output=output/training/mvp/attention/mvp_ltx_video_attn_lr_matrix_%A_%a.out
#SBATCH --error=output/training/mvp/attention/mvp_ltx_video_attn_lr_matrix_%A_%a.err
...
WANDB_GROUP="${WANDB_GROUP:-mvp_temporal_attn_ltx_video_lr_matrix}"
WANDB_NAME="${WANDB_NAME:-mvp_ltx_video_layer_${PROBE_LAYER}_lr_${LR_TAG}}"
