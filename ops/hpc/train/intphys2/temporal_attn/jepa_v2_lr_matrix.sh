#!/bin/bash
# Train IntPhys2 temporal_attn probes with jepa_v2 features over a fixed
# (layer, learning-rate) matrix.
#
# Usage:
#   sbatch ops/hpc/train/intphys2/temporal_attn/jepa_v2_lr_matrix.sh
#
# Default matrix:
#   layers = 10, 20, 30, 40
...
#   column -s, -t < artifacts/probes/intphys2/intphys2_probe_temporal_attn_jepa_v2_vitg_384_lr_matrix/train_eval_summary.csv

#SBATCH --partition=gpu_a100
#SBATCH --job-name=intphys2_jepa_v2_attn_lr_matrix
#SBATCH --array=0-15
#SBATCH --ntasks=1
#SBATCH --gpus=1
...
#SBATCH --output=output/training/intphys2/attention/intphys2_jepa_v2_attn_lr_matrix_%A_%a.out
#SBATCH --error=output/training/intphys2/attention/intphys2_jepa_v2_attn_lr_matrix_%A_%a.err

set -euo pipefail

DATASET_NAME="${DATASET_NAME:-intphys2}"
PROBE_NAME="${PROBE_NAME:-temporal_attn}"
BACKBONE_NAME="${BACKBONE_NAME:-jepa_v2}"
BACKBONE_VARIANT="${BACKBONE_VARIANT:-vitg_384}"
...
GROUP_SUBDIR="${GROUP_SUBDIR:-intphys2_probe_temporal_attn_jepa_v2_vitg_384_lr_matrix}"
WANDB_GROUP="${WANDB_GROUP:-intphys2_temporal_attn_jepa_v2_lr_matrix}"
