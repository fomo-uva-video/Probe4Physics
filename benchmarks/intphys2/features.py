...
class IntPhys2ChunkedTokenStore:
    def __init__(
        self,
        *,
        cache_dir: Path,
    ...
    def _load_chunk_payload(self, chunk_position: int) -> dict[str, Any]:
        ...
        if not isinstance(payload, dict):
            raise FeatureCacheError(
                f"Chunked token payload is not a dictionary: {self._chunks[chunk_position].tokens_path}"
            )
...
def run_intphys2_feature_extraction(
    ...
    *,
    ...
    with _resume_lock(resume_paths, stale_seconds=feature_cfg["lock_stale_seconds"]):
        ...
        index_rows, pooled_by_layer, _ = _materialize_from_chunks(
            state=state,
            include_pooled=bool(feature_cfg["include_pooled"]),
...
def load_feature_cache_for_config(
    ...
    *,
    ...
    token_store: IntPhys2ChunkedTokenStore | None = None
    ...
    if load_tokens and str(manifest.get("files", {}).get("tokens", "")):
        ...
        token_store = IntPhys2ChunkedTokenStore(cache_dir=paths.cache_dir, manifest=manifest, index=frame)
